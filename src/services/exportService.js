...
export const HEADERS_23 = [
  ...
  'Nama Pelapor', 'Timestamp Pengumpulan',
];
...
export const logsToRows = (logs) => logs.map((log) => [
  ...
  log.reporterName || '', log.submissionTimestamp || '',
]);

export const copyLogsAsTSV = async (logs) => {
  const text = [HEADERS_23, ...logsToRows(logs)]
    .map((row) => row.join('\t'))
    .join('\n');
...
export const exportLogsAsCSV = (logs) => {
  const csv = [HEADERS_23, ...logsToRows(logs)]
